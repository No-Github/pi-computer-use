...
impl AtspiClient {
    pub async fn connect() -> Result<Self, ProtocolError> {
        ...
        let bus = Proxy::new(&session, "org.a11y.Bus", "/org/a11y/bus", "org.a11y.Bus")
            .await
            .map_err(unavailable)?;
        let address: String = bus.get_property("Address").await.map_err(unavailable)?;
    ...
    async fn application_pid(&self, accessible: &AccessibleRef) -> u64 {
        let Ok(proxy) = self.proxy(accessible, "org.a11y.atspi.Application").await else {
            return 0;
        };
        proxy.get_property::<i32>("Id").await.unwrap_or(0).max(0) as u64
    }
    ...
    pub async fn press(&self, node: &NodeSnapshot) -> Result<bool, ProtocolError> {
        ...
        proxy.call("DoAction", &(0i32)).await.map_err(atspi_error)
    }
...
pub fn outline_json(
    ...
    for node in nodes.iter().rev() {
        let rect = node
            .bounds
            .as_ref()
            .map(|r| json!({"x":r.x - _root.frame.as_ref().map(|f| f.x).unwrap_or(0),"y":r.y - _root.frame.as_ref().map(|f| f.y).unwrap_or(0),"w":r.width,"h":r.height}))
